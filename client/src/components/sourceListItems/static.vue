<template>
  <div>
    <div>Loaded {{dataSize}} bytes of static data from {{dataURL}} according to
manifest at {{endpoint}}</div>

    <!-- Query options advertised by the source's manifest (doc/sources.rst,
         "Query options") -- one input per option, appended to the data-url
         query string on every (re)fetch; see connection.js's
         set_resource_query_value/load_resource_data. -->
